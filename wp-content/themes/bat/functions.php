...
class Nav_Walker_Nav_Menu extends Walker_Nav_Menu {
     function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
        ...
        $item_output .= str_replace('%s', $item->title, $args->link_before) . apply_filters( 'the_title', $item->title, $item->ID ) . $args->link_after;
...
add_action( 'optionsframework_custom_scripts', 'optionsframework_custom_scripts' );
 ...
 register_post_type('trampoline', array(
      'labels' => array('name' => __('Trampoline'),
                 'singular_name' => __('Trampoline'),
                 'add_new_item' => __('Add New Trampoline'),
                 'edit_item' => __('Edit Trampoline'),
           ),
          'public' => true,
		  '_builtin' => false,
		  'query_var' => true,
		  'rewrite' => array('slug' => 'trampoline', 'with_front' => false),
          'show_ui' => true,
          'supports' => array('title','editor','thumbnail', 'page-attributes')
  ));
  ...
  $args = array(
      ...
      'update_count_callback' => '_update_post_term_count',
      'query_var'             => true,
      'rewrite'               => array( 'slug' => 'trampoline-cat' )
  );

  register_taxonomy( 'trampoline_cat', 'trampoline', $args );
