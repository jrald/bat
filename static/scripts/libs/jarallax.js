...
Jarallax.prototype.setProgress = function (a) {
    if (a > 1) {
        a = 1
    } else if (a < 0) {
        a = 0
    ...
    for (j = 0; j < this.defaultValues.length; j++) {
        this.defaultValues[j].activate(a)
    }
    for (k = 0; k < this.animations.length; k++) {
        this.animations[k].activate(a)
    }
    for (l = 0; l < this.controllers.length; l++) {
        this.controllers[l].update(a)
    }
};
...
Jarallax.prototype.addAnimation = function (a, b) {
    ...
    if (Jarallax.isValues(b)) {
        for (var c = 0; c < b.length - 1; c++) {
            if (b[c] && b[c + 1]) {
                if (b[c]["progress"] && b[c + 1]["progress"]) {
                    this.animations.push(new JarallaxAnimation(a, b[c], b[c + 1]))
                } else {
...
JarallaxAnimation = function (a, b, c) {
    this.progress = 0;
    this.selector = a;
    this.startValues = b;
    this.endValues = c
            ...
            for (i in this.startValues) {
                if (i != "progress" && i != "style" && i != "event") {
                    if (undefined != this.endValues[i]) {
                        ...
                        var j = a - b;
                        var k = g - f;
                        var l = Jarallax.EASING[d["easing"]](j, f, k, h, d["easing"]["power"]);
                        l += e;
